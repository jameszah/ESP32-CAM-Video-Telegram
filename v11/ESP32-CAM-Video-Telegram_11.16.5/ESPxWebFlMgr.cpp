...
void ESPxWebFlMgr::begin() {
  ...
  // fileManager->on("/i", HTTP_GET, fileManagerFileListInsert); // use bind as the are members of object
  ...
  Serial.println("File Manger begin ...");
  fileManager->begin();
}
    ...
    */
    if ( (_ViewSysFiles) || (allowAccessToThisFile(fn)) ) {
      ...
      */
      ...
      if ( (fn.endsWith(".avi") ) )
      {
        fc += "<button title=\"Edit\" onclick=\"editavi('" + subdir + '/' + fn + "')\" class=\"b\">E</button> ";
      }

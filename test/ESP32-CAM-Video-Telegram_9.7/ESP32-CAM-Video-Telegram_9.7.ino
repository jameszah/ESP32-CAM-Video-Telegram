/*******************************************************************
  ...
  Oct 27, 2022 very 9.7
  - add the radu code to keep the shutter open for a long time in very low light
  - see the radu.cpp tab for more info
  - add flash photo option
  ...
  "C:\\ArduinoPortable\\arduino-1.8.19\\portable\\packages\\esp32\\hardware\\esp32\\2.0.4/tools/gen_esp32part.exe" -q "C:\\Users\\James\\AppData\\Local\\Temp\\arduino_build_232834/partitions.csv" "C:\\Users\\James\\AppData\\Local\\Temp\\arduino_build_232834/ESP32-CAM-Video-Telegram_9.6.ino.partitions.bin"
  ...
  "C:\\ArduinoPortable\\arduino-1.8.19\\portable\\packages\\esp32\\tools\\xtensa-esp32-elf-gcc\\gcc8_4_0-esp-2021r2-patch3/bin/xtensa-esp32-elf-size" -A "C:\\Users\\James\\AppData\\Local\\Temp\\arduino_build_232834/ESP32-CAM-Video-Telegram_9.6.ino.elf"
...
static const char vernum[] = "pir-cam 9.6";
...
int quality = 10;
int qualityconfig = 3;  // from 5 to accomodate radu
...
void handleNewMessages(int numNewMessages) {
  ...
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if (text == "/status") {
      ...
      Serial.printf("brightness: %d\n", ss->status.brightness);
      Serial.printf("awb_gain: %d\n", ss->status.awb_gain);
      Serial.printf("aec: %d\n", ss->status.aec);
      Serial.printf("aec2: %d\n", ss->status.aec2);
      Serial.printf("agc: %d\n", ss->status.agc);
      Serial.printf("agc_gain: %d\n", ss->status.agc_gain);
      Serial.printf("gainceiling: %d\n", ss->status.gainceiling);

      //s->set_exposure_ctrl(s, 1)
      ss->set_reg(ss, 0xff, 0xff, 0x01); //banksel
      int light = ss->get_reg(ss, 0x2f, 0xff);
      Serial.printf("Light is %d\n", light);
    ...
    if ( text == "/photo" || text == "/caption"  || text == "/fphoto") {
